...
if command -v code >/dev/null 2>&1; then
  code --install-extension gl1tch.copper-theme --force >/dev/null 2>&1 ||
    code --install-extension "$repo"/vscode/copper-theme-*.vsix --force >/dev/null
  say "vscode   Copper installed — pick it under Preferences: Color Theme"
else
  say "vscode   'code' CLI not found — install vscode/copper-theme-*.vsix by hand"
fi
